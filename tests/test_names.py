...
def test_a_single_trailing_letter_after_cjk_is_not_damage():
    # 华为数据中心-B is a real building designator. The truncation leaves the surviving
    # high 12 bits of one character, which is two base64 characters, never one.
    assert truncated_tail("华为数据中心B") == ""
    assert truncated_tail("华为数据中心BX") != ""
...
def test_a_nul_padded_field_is_still_scanned_for_damage():
    ...
    assert is_damaged("快手智能云乌兰察布数据中X8\x00\x00")
...
def test_padding_never_reaches_the_stored_name():
    ...
    result = build_names([("zh", "阿里巴巴数据中心\x00\x00"), ("en", "  Alibaba \t")])
    assert result.names == {"zh": "阿里巴巴数据中心", "en": "Alibaba"}
...
def test_cjk_detection_covers_the_ranges_that_matter():
    ...
    assert looks_chinese("Alibaba 数据中心")
    assert not looks_chinese("Alibaba Data Center")
...
def test_named_languages_are_kept_as_given():
    result = build_names([("zh", "阿里巴巴数据中心"), ("en", "Alibaba Data Center")])
    assert result.names == {"zh": "阿里巴巴数据中心", "en": "Alibaba Data Center"}
...
def test_damaged_names_are_counted_but_published_by_default():
    result = build_names([("zh", "快手智能云乌兰察布数据中X8"), ("en", "Kuaishou Data Center")])
    ...
    assert result.names["zh"] == "快手智能云乌兰察布数据中X8"
    assert result.names["en"] == "Kuaishou Data Center"


def test_skipping_damaged_names_omits_only_the_damaged_language():
    result = build_names(
        [("zh", "快手智能云乌兰察布数据中X8"), ("en", "Kuaishou Data Center")],
        skip_damaged=True,
    )
    assert result.names == {"en": "Kuaishou Data Center"}
    assert result.damaged == ("zh",)
